...
const initialMessages: ChatMessage[] = [
  { id: "1", username: "ozymandioz", message: "do you have any discount?", color: "text-amber-400" },
  { id: "2", username: "minameowmeow", message: "how much is this?", color: "text-pink-400" },
]
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="bg-[#1a1a1a] rounded-b-lg overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="font-semibold text-white text-sm">Host info/Name</span>
                {getHostLabel() && (
                  <Badge variant="secondary" className="text-xs px-1.5 py-0 bg-white/20 text-white">
                    {getHostLabel()}
                  </Badge>
                )}
              </div>
        ...
        <div className="relative aspect-[9/12] max-h-[400px] bg-[#1a1a1a]">
          ...
          <div className="absolute right-4 bottom-20 w-12">
            {hearts.map((heart) => (
              <div key={heart.id} className="absolute animate-float-up text-red-500" style={{ right: `${heart.x}px` }}>
          ...
          <div className="absolute bottom-16 left-3 right-16 space-y-1">
            {messages.slice(-3).map((msg) => (
              <div key={msg.id} className="text-sm bg-black/40 backdrop-blur-sm rounded px-2 py-1">
                <span className={`font-medium ${msg.color}`}>{msg.username}:</span>
      ...
      <div className="bg-muted rounded-lg p-3">
        <p className="text-xs text-muted-foreground mb-2 font-medium">Experiment: Host Condition</p>
        <div className="flex flex-wrap gap-2">
          {(["human", "ai-avatar", "stylized", "voice-only"] as HostType[]).map((type) => (
            <Button
              key={type}
